...
public class CleanupUnusedFilesCommandHandlerTests : TestBase
{
    ...
    /// <summary>
    /// Kiểm tra xem các tệp không sử dụng có được xóa thành công.
    /// </summary>
    [Fact]
    public async Task Handle_Should_DeleteUnusedFiles_Successfully()
    {
        // Arrange
    ...
    /// <summary>
    /// Kiểm tra xem các tệp đang sử dụng có không bị xóa.
    /// </summary>
    [Fact]
    public async Task Handle_Should_NotDeleteUsedFiles()
    {
        // Arrange
        var userId = Guid.NewGuid().ToString();
        var userProfileId = Guid.NewGuid();
        ...
        // Assert
        result.IsSuccess.Should().BeTrue();
        _context.FileMetadata.Should().HaveCount(1);
        _context.FileMetadata.First().Id.Should().Be(usedFile.Id);
