...
public class GetRelationshipByIdQueryHandlerTests : TestBase
{
    ...
    {
        _handler = new GetRelationshipByIdQueryHandler(_context, _mapper);
    }
    ...
    /// <summary>
    /// Kiểm tra xem handler có trả về đúng mối quan hệ khi tìm thấy bằng ID.
    /// </summary>
    [Fact]
    public async Task Handle_Should_ReturnRelationship_When_Found()
    {
        // Arrange
        ...
        var query = new GetRelationshipByIdQuery(relationship.Id);
        ...
        result.Value!.Id.Should().Be(relationship.Id);
        result.Value.Type.Should().Be(relationship.Type);
    }

    /// <summary>
    /// Kiểm tra khi không tìm thấy mối quan hệ.
    /// </summary>
    [Fact]
    public async Task Handle_Should_ReturnFailure_When_NotFound()
    {
        // Arrange
        var nonExistentRelationshipId = Guid.NewGuid();
        var query = new GetRelationshipByIdQuery(nonExistentRelationshipId);
        ...
        result.Error.Should().Contain($"Relationship with ID {nonExistentRelationshipId} not found.");
    }
}
