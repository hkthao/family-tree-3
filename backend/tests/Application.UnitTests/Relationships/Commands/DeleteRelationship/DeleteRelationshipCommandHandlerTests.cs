...
public class DeleteRelationshipCommandHandlerTests : TestBase
{
    ...
    {
        _handler = new DeleteRelationshipCommandHandler(_context, _mockAuthorizationService.Object, _mockMediator.Object);
    }
    ...
    /// <summary>
    /// Kiểm tra xem một mối quan hệ có được xóa thành công.
    /// </summary>
    [Fact]
    public async Task Handle_Should_DeleteRelationship_Successfully()
    {
        // Arrange
        ...
        var command = new DeleteRelationshipCommand(existingRelationship.Id);
        ...
        var deletedRelationship = await _context.Relationships.FindAsync(existingRelationship.Id);
    ...
    /// <summary>
    /// Kiểm tra khi mối quan hệ cần xóa không tồn tại.
    /// </summary>
    [Fact]
    public async Task Handle_Should_ReturnFailure_When_RelationshipNotFound()
    {
        ...
        var command = new DeleteRelationshipCommand(Guid.NewGuid()); // Non-existent relationship ID
    ...
    /// <summary>
    /// Kiểm tra khi người dùng không được ủy quyền để xóa mối quan hệ.
    /// </summary>
    [Fact]
