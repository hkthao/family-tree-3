...
public class GetUserProfileByExternalIdQueryHandlerTests : TestBase
{
    ...
    {
        _handler = new GetUserProfileByExternalIdQueryHandler(_context, _mapper);
    }
    ...
    /// <summary>
    /// Kiểm tra xem hồ sơ người dùng có được trả về thành công khi tìm thấy bằng ExternalId.
    /// </summary>
    [Fact]
    public async Task Handle_Should_ReturnUserProfile_When_Found()
    {
        // Arrange
        ...
        var query = new GetUserProfileByExternalIdQuery { ExternalId = userId.ToString() };
    ...
    /// <summary>
    /// Kiểm tra khi không tìm thấy hồ sơ người dùng bằng ExternalId.
    /// </summary>
    [Fact]
    public async Task Handle_Should_ReturnFailure_When_NotFound()
    {
        ...
        var nonExistentExternalId = Guid.NewGuid().ToString();
        var query = new GetUserProfileByExternalIdQuery { ExternalId = nonExistentExternalId.ToString() };
