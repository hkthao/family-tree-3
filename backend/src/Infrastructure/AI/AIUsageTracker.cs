...
public class AIUsageTracker : IAIUsageTracker
{
    ...
    public AIUsageTracker(IMemoryCache memoryCache, ILogger<AIUsageTracker> logger, IOptions<AIConfig> aiConfig)
    {
    ...
    public async Task<Result<bool>> CheckAndRecordUsageAsync(Guid userProfileId, int tokensUsed, CancellationToken cancellationToken = default)
    {
        ...
        if (currentUsageCount >= _aiConfig.DailyUsageLimit)
        {
        ...
        if (tokensUsed > _aiConfig.MaxTokensPerRequest)
        {
            _logger.LogWarning("Max tokens per request exceeded for user {UserProfileId}. Requested: {TokensUsed}, Max: {MaxTokens}.", userProfileId, tokensUsed, _aiConfig.MaxTokensPerRequest);
    ...
    public Task<Result<int>> GetRemainingDailyTokensAsync(Guid userProfileId, CancellationToken cancellationToken = default)
    {
        ...
        var remaining = _aiConfig.DailyUsageLimit - currentTokenCount;
