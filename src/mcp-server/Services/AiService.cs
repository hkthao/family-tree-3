                ...
                foreach (var toolCall in toolCalls)
                {
                    ...
                    switch (toolCall.FunctionName)
                    {
                        case "search_family":
                            ...
                            {
                                var args = JsonSerializer.Deserialize<JsonElement>(toolCall.FunctionArgs);
                                var query = args.TryGetProperty("query", out var q) ? q.GetString() : string.Empty;
                                result = await _familyTreeBackendService.SearchFamiliesAsync(jwtToken, query ?? string.Empty);
                            }
                            break;
                        ...
                        case "search_members":
                            ...
                            else
                            {
                                ...
                                result = await _familyTreeBackendService.SearchMembersAsync(jwtToken, query ?? string.Empty, familyId);
                            }
                            break;
                        ...
                        case "search_events":
                            ...
                            else
                            {
                                ...
                                result = await _familyTreeBackendService.SearchEventsAsync(jwtToken, query ?? string.Empty, familyId, startDate, endDate);
                            }
                            break;
