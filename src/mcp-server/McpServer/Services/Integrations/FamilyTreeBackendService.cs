...
namespace McpServer.Services.Integrations;
...
public class FamilyTreeBackendService
{
    ...
    public virtual async Task<List<FamilyDto>?> SearchFamiliesAsync(string jwtToken, string query)
    {
        try
        {
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", jwtToken);
            ...
            return result?.Value?.Items?.Select(f => new FamilyDto { Id = f.Id.GetHashCode(), Name = f.Name, History = f.Description }).ToList();
        }
        catch (HttpRequestException ex)
        {
    ...
    /// </summary>
    public virtual async Task<FamilyDto?> GetFamilyByIdAsync(Guid id, string jwtToken)
    {
        try
        {
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", jwtToken);
            ...
            return externalFamilyDto != null ? new FamilyDto { Id = externalFamilyDto.Id.GetHashCode(), Name = externalFamilyDto.Name, History = externalFamilyDto.Description } : null;
        }
        catch (HttpRequestException ex)
        {
