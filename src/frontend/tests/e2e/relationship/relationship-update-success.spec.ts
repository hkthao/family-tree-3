...
test.describe('Relationship Management - Update Relationship - Success Case', () => {
  ...
  test('should allow a user to update an existing relationship successfully', async ({ page }) => {
    ...
    console.log('Điều hướng về danh sách mối quan hệ và tìm kiếm lại.');
    await page.getByRole('link', { name: 'Quản lý Quan hệ' }).click();
    await page.waitForLoadState('networkidle');
    ...
    console.log('Xác minh mối quan hệ đã cập nhật hiển thị trong danh sách.');
