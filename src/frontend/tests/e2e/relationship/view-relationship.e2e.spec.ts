...
test.describe('Relationship Management - View Relationship', () => {
  ...
  test('should allow a user to view relationship details', async ({ page }) => {
    ...
    const member1LastName = `ViewRel1 ${new Date().getTime()}`;
    const member2FirstName = 'Thành viên 2';
    const member2LastName = `ViewRel2 ${new Date().getTime()}`;
    ...
    // 4. Click on the relationship to view details
    console.log('Click vào mối quan hệ để xem chi tiết.');
    await page.locator('tr').filter({ hasText: relationshipType }).nth(0).click();
    await page.waitForLoadState('networkidle');
    console.log('Đã vào trang chi tiết mối quan hệ.');

    // 5. Assert details
    console.log('Xác minh thông tin chi tiết mối quan hệ.');
    // We can't assert specific member names as we are picking random ones.
    // Instead, we assert that the relationship type is displayed correctly.
