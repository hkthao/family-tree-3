...
test.describe('Relationship Management - Update Relationship - Validation Case', () => {
  ...
  test('should show validation errors for empty required fields on update', async ({ page }) => {
    ...
    // For select fields, we can't truly make them empty, but we can try to select an invalid/empty option if available, or just assert if they are required.
    // For this test, we will assume that the select fields are already filled from the creation step and focus on other potential validation.
    // If there were text inputs that could be cleared, we would do so here.

    // For demonstration, let's assume 'relationship-order-input' is a required field that can be cleared.
    await fillVuetifyInput(page, 'relationship-order-input', '');

    console.log('Click nút "Lưu mối quan hệ" mà không điền các trường bắt buộc.');
    await page.getByTestId('button-save').click();
