...
public class MemberDeletedEventHandlerTests
{
    ...
    /// <summary>
    /// 🎯 Mục tiêu của test: Xác minh rằng handler xử lý sự kiện MemberDeletedEvent một cách chính xác,
    /// bao gồm ghi log, ghi lại hoạt động người dùng, xuất bản thông báo và xóa dữ liệu khỏi dịch vụ tìm kiếm toàn cầu.
    /// ⚙️ Các bước (Arrange, Act, Assert):
    ///    - Arrange: Tạo một Member và một MemberDeletedEvent. Thiết lập _mockUser.Id.
    ...
    /// cần thiết sau khi thành viên bị xóa đều được thực hiện một cách chính xác và đầy đủ.
    /// </summary>
    [Fact]
    public async Task Handle_ShouldPerformAllRequiredActions_WhenMemberDeletedEventIsHandled()
    {
        ...
        _mockNotificationPublisher.Setup(p => p.PublishNotificationForEventAsync(It.IsAny<MemberDeletedEvent>(), It.IsAny<CancellationToken>()))
                                  .Returns(Task.CompletedTask);
        _mockGlobalSearchService.Setup(g => g.DeleteEntityFromSearchAsync(
            It.IsAny<string>(),
            It.IsAny<string>(),
        ...
        _mockLogger.Verify(
            x => x.Log(
                LogLevel.Information,
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, t) => v.ToString()!.Contains($"Member {notification.Member.FullName} ({notification.Member.Id}) was successfully deleted.")),
                It.IsAny<Exception>(),
                It.IsAny<Func<It.IsAnyType, Exception?, string>>()),
            Times.Once);
