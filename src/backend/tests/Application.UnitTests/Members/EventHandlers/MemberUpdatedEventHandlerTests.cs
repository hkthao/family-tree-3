...
public class MemberUpdatedEventHandlerTests : TestBase
{
    private readonly Mock<ILogger<MemberUpdatedEventHandler>> _mockLogger;
    ...
    public MemberUpdatedEventHandlerTests() : base()
    {
        _mockLogger = new Mock<ILogger<MemberUpdatedEventHandler>>();
        ...
        _handler = new MemberUpdatedEventHandler(
            _mockLogger.Object,
    ...
    /// <summary>
    /// 🎯 Mục tiêu của test: Xác minh rằng handler xử lý sự kiện MemberUpdatedEvent một cách chính xác,
    /// bao gồm ghi log, ghi lại hoạt động người dùng, xuất bản thông báo và cập nhật dịch vụ tìm kiếm toàn cầu.
    /// ⚙️ Các bước (Arrange, Act, Assert):
    ///    - Arrange: Tạo một Member và một MemberUpdatedEvent. Thiết lập _mockUser.Id.
    ...
    /// cần thiết sau khi thông tin thành viên được cập nhật đều được thực hiện một cách chính xác và đầy đủ.
    /// </summary>
    [Fact]
    public async Task Handle_ShouldPerformAllRequiredActions_WhenMemberUpdatedEventIsHandled()
    {
        // Arrange
        ...
        _mockNotificationPublisher.Setup(p => p.PublishNotificationForEventAsync(It.IsAny<MemberUpdatedEvent>(), It.IsAny<CancellationToken>()))
        ...
        _mockLogger.Verify(
            x => x.Log(
                LogLevel.Information,
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, t) => v.ToString()!.Contains($"Member {notification.Member.FullName} ({notification.Member.Id}) was successfully updated.")),
                It.IsAny<Exception>(),
                It.IsAny<Func<It.IsAnyType, Exception?, string>>()),
            Times.Once);
        ...
        _mockMediator.Verify(m => m.Send(
            It.Is<RecordActivityCommand>(cmd =>
                ...
                cmd.ActivitySummary.Contains($"Updated member '{notification.Member.FullName}'") &&
