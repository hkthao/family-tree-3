        ...
        else if (currentUserId.HasValue && currentUserId != Guid.Empty)
        {
            ...
            Query.Where(mi => mi.Family != null &&
                              (mi.Family.CreatedBy == currentUserId.Value.ToString() ||
                               ...
                               mi.Family.Visibility == FamilyVisibility.Public.ToString()));
        }
        else // Not admin and not authenticated
        {
            // Unauthenticated users should only see memory items belonging to public families.
            Query.Where(mi => mi.Family != null && mi.Family.Visibility == FamilyVisibility.Public.ToString());
        }
    }
}
