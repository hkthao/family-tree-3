...
public class ActivateVoiceProfileCommandHandlerTests : TestBase
{
    [Fact]
    public async Task Handle_ShouldActivateVoiceProfileAndReturnSuccess()
    {
        using var context = GetApplicationDbContext(); // Get fresh context
        var handler = new ActivateVoiceProfileCommandHandler(context, _mapper);
        ...
        voiceProfile.Archive(); // Set to archived initially
        ...
        var activatedVoiceProfile = await context.VoiceProfiles.FindAsync(voiceProfile.Id);
        ...
        activatedVoiceProfile.Should().NotBeNull();
        activatedVoiceProfile!.Status.Should().Be(VoiceProfileStatus.Active);
    }
