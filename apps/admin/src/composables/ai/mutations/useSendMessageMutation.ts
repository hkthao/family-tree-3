...
import type { ApiError, AiChatMessage, Result } from '@/types';
  ...
  return useMutation<AiChatMessage, ApiError, SendMessagePayload>(
    {
      mutationFn: async (payload: SendMessagePayload) => {
        const result: Result<string, ApiError> = await chatService.sendMessage(payload.familyId, payload.sessionId, payload.message);
        if (result.ok) {
          return { sender: 'ai', text: result.value } as AiChatMessage;
        } else {
          throw result.error;
        }
      },
