...
export function useMemberFormComposable(options: UseMemberFormOptions) {
  ...
  const formData = reactive<MemberAddDto | MemberUpdateDto>(
    (options.initialMemberData
      ...
      : {
        ...
        lunarDateOfDeath: { day: undefined, month: undefined, isLeapMonth: false },
        gender: Gender.Male,
  ...
  const getFormData = () => {
    if (formData.lunarDateOfDeath && formData.lunarDateOfDeath.day && formData.lunarDateOfDeath.month) {
      if (formData.lunarDateOfDeath.isLeapMonth == undefined)
        formData.lunarDateOfDeath.isLeapMonth = false;
    }
    else {
      formData.lunarDateOfDeath = undefined;
    }
    return formData;
  };
