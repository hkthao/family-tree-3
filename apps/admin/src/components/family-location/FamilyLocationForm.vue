    ...
    <v-row>
      ...
      <v-col cols="12" md="12">
        <v-text-field v-model="form.name" :label="t('familyLocation.form.name')" :rules="rules.name" required
          data-testid="name" prepend-inner-icon="mdi-map-marker-account"></v-text-field>
      </v-col>
      <v-col cols="12">
        <v-textarea v-model="form.description" :label="t('familyLocation.form.description')" rows="2"
          data-testid="description" prepend-inner-icon="mdi-text-box-outline"></v-textarea>
      </v-col>
      <v-col cols="12">
        <v-text-field v-model="form.address" :label="t('familyLocation.form.address')"
          data-testid="address" prepend-inner-icon="mdi-map-marker"></v-text-field>
      </v-col>
      <v-col cols="12" md="6">
        <v-text-field v-model.number="form.latitude" :label="t('familyLocation.form.latitude')" type="number" step="any"
          data-testid="latitude" :rules="rules.latitude" prepend-inner-icon="mdi-latitude"></v-text-field>
      </v-col>
      <v-col cols="12" md="6">
        <v-text-field v-model.number="form.longitude" :label="t('familyLocation.form.longitude')" type="number"
          step="any" data-testid="longitude" :rules="rules.longitude" prepend-inner-icon="mdi-longitude"></v-text-field>
      </v-col>
      <v-col cols="12" md="6">
        <v-select v-model="form.accuracy" :items="locationAccuracyOptions" :label="t('familyLocation.form.accuracy')"
          :rules="rules.accuracy" required data-testid="accuracy" item-title="title" item-value="value"
      ...
      <v-col cols="12" md="6">
        <v-select v-model="form.source" :items="locationSourceOptions" :label="t('familyLocation.form.source')"
          :rules="rules.source" required data-testid="source" item-title="title" item-value="value"
