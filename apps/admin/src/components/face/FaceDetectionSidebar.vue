<template>
  <div>
    <v-alert v-if="faces.length > 0" density="compact" type="info" variant="tonal" class="my-2">
    ...
    <v-list>
      <v-list-item v-for="face in faces" :key="face.id" :class="{
        ...
        <template v-slot:append>
          ...
          <v-icon v-if="face.status === 'unrecognized'" color="warning">mdi-alert-circle</v-icon>
          <v-icon v-if="face.status === 'newly-labeled'" color="info">mdi-tag</v-icon>
