<template>
  <v-navigation-drawer v-model="mediaPickerStore.drawer" location="right" width="650" temporary
    ...
    <v-card-text>
      <VFileUpload v-if="canManageFamilyMedia" v-model="fileToUpload" :label="t('familyMedia.form.fileLabel')"
        prepend-icon="mdi-paperclip" :multiple="true" show-size :clearable="true"
        @update:modelValue="handleFilesUpdate"></VFileUpload>
      <v-btn v-if="canManageFamilyMedia && fileToUpload.length > 0" color="primary" class="mt-2"
      ...
      <MediaPickerContent v-if="mediaPickerStore.familyId" :family-id="mediaPickerStore.familyId"
        ...
        @update:selectedMedia="handleSelectionUpdate" :allow-delete="canManageFamilyMedia"
        :allow-upload="canManageFamilyMedia" :initial-media-type="mediaPickerStore.initialMediaType" />
    </v-card-text>
...
const canManageFamilyMedia = computed(() => {
  if (!mediaPickerStore.familyId) return false;
  return state.isAdmin.value || state.isFamilyManager.value(mediaPickerStore.familyId);
});
