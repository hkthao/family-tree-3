...
from ..schemas.vectors import AddVectorRequest, UpdateVectorRequest, DeleteVectorRequest, RebuildVectorRequest, VectorData
...
async def add_vectors(request: AddVectorRequest):
    ...
    """
    try:
        # Assuming all vectors in a request belong to the same family for table naming
        if not request.vectors:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="No vectors provided.")
        
        family_id = request.vectors[0].family_id
        table_name = lancedb_service._get_table_name(family_id)
        
        # Ensure table exists before adding. create_dummy_table also handles recreation.
        lancedb_service.create_dummy_table(family_id) 
            
        lancedb_service.add_vectors(table_name, request.vectors)
        return {"message": f"Successfully added {len(request.vectors)} vectors to family {family_id}."}
...
async def update_vectors(request: UpdateVectorRequest):
    ...
    try:
        ...
        if table_name not in lancedb_service.db.table_names():
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Family table '{table_name}' not found.")
        
        lancedb_service.update_vectors(table_name, request)
        return {"message": f"Successfully updated vectors for family {request.family_id}."}
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(e))
...
async def delete_vectors(request: DeleteVectorRequest):
    ...
    try:
        ...
        return {"message": f"Successfully deleted vectors for family {request.family_id}."}
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(e))
...
async def rebuild_vectors(request: RebuildVectorRequest):
    ...
    try:
        ...
        return {"message": f"Rebuild process triggered for family {request.family_id}."}
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(e))
