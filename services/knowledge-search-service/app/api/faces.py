...
from ..models.face_models import (
    ...
    SearchFaceRequest, SearchFacesResponse, SearchResult
)
router = APIRouter()
# Dependencies
...
async def search_faces(
    ...
    try:
        ...
        if not query_embedding:
            ...
            query_embedding = face_embed_service.embed_image_url(
                request.image_url
            )
            logger.info(f"Generated query embedding from image_url for search.")
        ...
        logger.exception(f"Đã xảy ra lỗi nội bộ khi tìm kiếm khuôn mặt: {e}") # Log full traceback
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def update_face(
    ...
    try:
        ...
        if "original_image_url" in update_data and "embedding" not in update_data:
            ...
            logger.info(f"Generated new embedding from original_image_url for face_id: {face_id}")
