...
async def resize_image(
    ...
    height: Optional[int] = Query(
        None,
        description="Desired height for the resized image (optional, aspect ratio will be maintained if not provided)",
    ),
):
    logger.info(
        "Received request to resize image. Filename: %s, Target Width: %s, Target Height: %s",
        file.filename,
        width,
        height,
    )
    ...
    try:
        ...
        if height is None:
            ...
            logger.info(
                f"Resizing image to {size[0]}x{size[1]} (maintaining aspect ratio)."
            )
            image.thumbnail(
                size, Image.LANCZOS
            )  # Use thumbnail to maintain aspect ratio and not upscale
        else:
            size = (width, height)
        ...
        buffered = io.BytesIO()
        image.save(
            buffered, format=image.format if image.format else "PNG"
